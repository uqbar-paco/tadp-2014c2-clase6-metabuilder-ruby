...
class ClaseConParametros
  def initialize un_parametro
    # nada
  end
end
...
describe 'Metabuilder' do
  ...
  it 'no puedo usar clases con constructores con parámetros' do
    expect {
      Metabuilder.new.
          set_target_class(ClaseConParametros).
          build
    }.to raise_error(InvalidConstructorError)
  end
  ...
  it 'puede agregar métodos con parámetros' do
    metabuilder = Metabuilder.config {
      ...
      property :raza
      property :edad
      conditional_method :rescata_a, proc { raza == "San Bernardo" }, proc {
          |nombre|
        "Rescate a #{nombre}!"
      }
    }
